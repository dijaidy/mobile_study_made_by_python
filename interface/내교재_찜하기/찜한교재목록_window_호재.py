...
class 찜한교재목록_window(찜한교재_manage_user_information):
    def __init__(self):
        ...
        # 큰 분류기준선택버튼만들기
        self.sort_by_acheivement = Button(
            self.window,
            text="성취도순 분류",
            font=("배달의민족 주아", 11),
            command=lambda: self.init_checkbox(standard="성취도순 분류"),
        )
        self.sort_by_subject = Button(
            self.window,
            text="과목별 분류",
            font=("배달의민족 주아", 11),
            command=lambda: self.init_checkbox(standard="과목별 분류"),
        )
        ...
        self.acheivement_bar = tk.Progressbar(
            self.window, maximum=100, length=175, variable=self.book_acheivement
        )
