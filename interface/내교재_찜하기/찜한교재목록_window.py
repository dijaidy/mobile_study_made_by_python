...
class 찜한교재목록_window(찜한교재_manage_user_information):
    def __init__(self):
        ...
        #이전 교재 보여주기 버튼
        self.prior_button=Button(self.window, text="이전 교재로 가기", font=menu_font, command=lambda: self.show_book_information(-1))
        self.prior_button.place(relx=0, y=750, height=50, width=200)

        #다음 교재 보여주기 버튼
        self.next_button=Button(self.window, text="다음 교재로 가기", font=menu_font, command=lambda: self.show_book_information(1))
        self.next_button.place(relx=1/2, y=750, height=50, width=200)

        #분류기준체크박스만들기
        self.combobox=Listbox(self.window, selectmode="extended", height=0, width=150)
        self.combobox.insert(0, "과목별 분류")
        self.combobox.insert(1, "성취도순 분류")
        self.combobox.place(x=0, y=0)

        #
        self.combobox1=Listbox(self.window, selectmode="extended", height=0, width=150)
        self.combobox1.pack()
        
        #분류기준확인버튼만들기
        self.check_standard=Button(self.window, text="확인 후 분류기준 선택하기", font=("배달의민족 주아", 10), command=lambda: self.init_checkbox(self.combobox.get(self.combobox.curselection()[0], self.combobox.curselection()[0])))
        self.check_standard.place(x=150, y=0)

        #최종확인버튼만들기
        self.showing_button=Button(self.window, text="확인 후 교재보기", font=("배달의민족 주아", 10), command=lambda: self.show_book_information(first_show_standard=self.combobox.get(self.combobox.curselection()[0], self.combobox.curselection()[0]), second_show_standard=self.combobox1.get(self.combobox1.curselection()[0], self.combobox1.curselction()[0])))
        self.showing_button.place(x=300, y=0)
    ...
    def init_checkbox(self, standard=""):
        if (standard=="과목별 분류"):
            ...
            self.combobox1.place(self.window, width=0, height=0, x=0, y=20)
    ...
    def show_book_information(self, index_moving=0, first_show_standard="", second_show_standard=""):  # 알라딘api에서 가져온 책 정보를 이용해 띄워줌
        # 찜한 책 모음 가져오기
        searching_result={}
        chosen_book_keys=self.chosen_book.keys()
        if first_show_standard=="과목별 분류":
            for i in chosen_book_keys:
                if self.chosen_book[i]["subject"]==second_show_standard:
                    searching_result.append(self.chosen_book[i])
        elif first_show_standard=="성취도순 분류":
            for i in chosen_book_keys:
                if int(second_show_standard.split('~')[0].replace("%", ""))<=self.chosen_book[i]["acheivement"]<=int(second_show_standard.split('~')[1].replace("%", "")):
                    searching_result.append(self.chosen_book[i])

        # 인덱스 조정
        self.image_index += index_moving
        if self.image_index < -1 * len(searching_result):
