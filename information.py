...
import json
class 찜한교재_manage_information:   #교재등록 밑 관리
    def __init__(self):
        self.chosen_book={}
    ...
    def save_chosen_book_to_file(self):   #교재정보를 파일에 저장
        with open('chosen_book_file.json', 'w', encoding='UTF-8') as out_file: 
            json.dump(self.chosen_book, out_file, ensure_ascii=False)
                

    def call_chosen_book_from_file(self):   #파일 불러오기
        with open('chosen_book_file.json', 'r') as out_file:
            self.chosen_book=json.load(out_file)

찜한교재_manage_information().call_chosen_book_from_file()